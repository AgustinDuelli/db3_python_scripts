USE master;
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'PARK_2_BUENOSAIRES_ENTRADAS')
    BEGIN
        CREATE DATABASE [PARK_2_BUENOSAIRES_ENTRADAS];
    END
GO

USE [PARK_2_BUENOSAIRES_ENTRADAS];
GO
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_TipoVisita_Categoria')
    BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_ItemVenta_TipoVisita')
    BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_Venta_ItemVenta')
    BEGIN
        ALTER TABLE [Venta]
